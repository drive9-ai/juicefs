...
func (s *tieredSQLStore) commitLargeRef(ctx context.Context, key string, size int64, checksum, payloadRef []byte) (uint64, error) {
	if size <= s.smallThreshold {
		return 0, fmt.Errorf("%w: large payload size %d does not exceed threshold %d", errTieredSQLUnsupported, size, s.smallThreshold)
	}
	if len(payloadRef) == 0 {
		return 0, errors.New("empty large payload ref")
	}
	var generation uint64
	err := s.withRetry(ctx, func(tx *sql.Tx) error {
		var err error
